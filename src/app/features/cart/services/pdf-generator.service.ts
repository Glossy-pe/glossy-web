...
export class PdfGeneratorService {
  ...
  generateOrderPdf(order: OrderResponse): void {
    ...
    const tableData = order.orderItems.map(item => [
      ...
      item.quantity.toString(),
      `S/. ${Number(item.productVariant.price).toFixed(2)}`,
      `S/. ${(Number(item.productVariant.price) * item.quantity).toFixed(2)}`
    ]);
    ...
    const subtotal = order.orderItems.reduce(
      (acc, item) => acc + Number(item.productVariant.price) * item.quantity, 0
    );
    ...
    doc.setTextColor(41, 37, 36);
    doc.text('TOTAL:', totalsX, currentY);
    doc.text(`S/. ${Number(order.total).toFixed(2)}`, pageWidth - 15, currentY, { align: 'right' });
